...
\lstset{
    ...
    commentstyle = \color[HTML]{808080}, %% Sets comment color.
    keywordstyle = \color[HTML]{A00080}\bfseries, %% Sets  keyword color.
    stringstyle = \color[HTML]{008000}, %% Sets  string color.
}
...
\begin{frame}
    ...
    \begin{tikzpicture}[x=\textwidth, y=\textheight]
        ...
        \visible<3->{
            \node[] at (.735,0.53) {\includegraphics[height=0.075\textwidth]{fig/logos/imagej/paintera.png}};
...
% this does not seem to work, just use external player instead
% \end{frame}
% \movie[width=\linewidth,height=\textheight]{BigDataViewer Demo}{vid/fafb-bdv.mp4}
% \begin{frame}
...
\begin{frame}
    ...
    \begin{tikzpicture}
        ...
        \node[yshift=-0\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/logos/python/python.png}};
        \node[yshift=-1\myYShift] (numpy-icon)
        {\includegraphics[height=\myHeight]{fig/logos/python/numpy.png}};
        \node[yshift=-2\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/logos/python/scipy.png}};
        \node[yshift=-3\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/logos/python/scikit-image.jpeg}};
        \node[yshift=-4\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/logos/python/matplotlib.pdf}};
        \node[yshift=-5\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/logos/imagej/imagej2.png}};
        \node[yshift=-6\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/logos/imagej/imglib2.png}};
        \node[yshift=-7\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/logos/imagej/paintera.png}};
        \node[yshift=-8\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/logos/imagej/fiji.png}};
        \node[yshift=-9\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/logos/imagej/mammut.png}};
        \node[yshift=-10\myYShift] (python-icon)
        {\includegraphics[height=\myHeight]{fig/bridge.pdf}};
    \end{scope}
    \begin{scope}[xshift=\myXShift,anchor=west,font=\tiny]
        \node[yshift=-0\myYShift]{\url{https://www.python.org}};
        \node[yshift=-1\myYShift]{\url{https://www.numpy.org}};
        \node[yshift=-2\myYShift]{\url{https://scipy.org}};
        \node[yshift=-3\myYShift]{\url{https://scikit-image.org}};
        \node[yshift=-4\myYShift]{\url{https://matplotlib.org}};
        \node[yshift=-5\myYShift]{\url{https://imagej.net/ImageJ2}};
        \node[yshift=-6\myYShift]{\url{https://imagej.net/ImgLib2}};
        \node[yshift=-7\myYShift]{\url{https://github.com/saalfeldlab/paintera}};
        \node[yshift=-8\myYShift]{\url{https://fiji.sc/}};
        \node[yshift=-9\myYShift]{\url{https://imagej.net/MaMuT}};
        \node[yshift=-10\myYShift]{\url{https://openclipart.org/detail/209218/firebog-bridge}};
    \end{scope}
    \end{tikzpicture}
\end{frame}
...
\begin{frame}[fragile]
    ...
    {%
        \begin{tikzpicture}
            \node[anchor=south west,xshift=0.75\linewidth] (qrcode) {\includegraphics[width=0.2\linewidth]{fig/slides-qr.png}};
            \node[%
            % rectangle,%
            % draw=HHMIGreenB,%
            % thick,%
            inner sep=0,
            xshift=0.0\linewidth,%
            anchor=south west,
            font=\footnotesize] (slides) {\url{https://github.com/hanslovsky/scipy-2019}};
            % \path[draw=black,thick] (qrcode.west) to[bend left=45] node[above,midway,sloped] {Slides} (qrcode.north);
            \path[%
            % draw=black,%
            % thick,%
            overlay,
            postaction={decorate,decoration={text along path,text
                    align=right,raise=-3.7ex,text={Link to Slides}}}%
            ]%
            (qrcode.center) circle (0.12\linewidth);
            % (qrcode.west) to[bend right=60] (qrcode.south) to[bend right=45]
            % (qrcode.east);
        \end{tikzpicture}
        % \large
        % \url{https://github.com/hanslovsky/scipy-2019}
    }%
    \\[1em]
\begin{lstlisting}[language=bash]
conda install -c conda-forge imglyb
...
\end{lstlisting}
    {    
        ...
        \urlScrSz{https://github.com/imagej/pyimagej}
    }
    \\[0.5em]
